...
/*
	...
	List query(String sql, RowMapper rm)
		: 여러개의 레코드를 반환하는 select게열의 쿼리문인 경우 사용한다.
	List query(String sql, Object[] args, RowMapper rm)
		: 인파라미터를 가진 여러개의 레코드를 반환한느 select계열의
			쿼리문인 경우 사용한다.
...
public class JDBCTemplateDAO {
	...
	public void write(final SpringBbsDTO springBbsDTO) {
		...
		/*
			...
			반드시 final로 선언하여 값의 변경이 불가능하게 처리 해야한다.
			final로 선언하지 않으면 에러가 발생한다.(Java의 문법(규칙))
		 */
		template.update(new PreparedStatementCreator() {
			...
			public PreparedStatement 
				...
				/*
					답변형 게시판에서 원본글은 경우 idx와 bgroup은 반드시
					일치해야 한다.
	...
	public SpringBbsDTO view(String idx) {
		...
		try {
			/*
				하나의 결과르 반환하는 select계열의 쿼리문을 실행할때는
	...
	public void reply(final SpringBbsDTO dto) {
		...
		/*
			...
			입력해야 한다.
			글쓰기(write)와 다른점은 원본글의 bgroup번호를 그대로 입력하는
			것이다.
		 */
		String sql = "INSERT INTO springboard "
