...
public class BbsController {
	...
	// 글쓰기 - 폼 에 진입시에는 get방식으로 처리
	@RequestMapping("/board/write.do")
	public String write(Model model) {
		return "07Board/write";
	}
	
	// 글쓰기처리 - post방식으로 전송되므로 value, method로 둘 다 명시했음.
	...
	public String writeAction(Model model,
		...
		/*
			redirect : 이동한 페이지경로(요청명) 와 같이하면 뷰를 호출하지 않고
						페이지 이동이 된다.
		 */
	...
	public String passwordAction(Model model, HttpServletRequest req) {
		...
		if(rowExist<=0) {
			...
			if(mode.equals("edit")) {
				...
				// mode가 delete인 경우 삭제처리
				model.addAttribute("req", req);
				//command = new DeleteActionCommand();
				command = deleteActionCommand;
				command.execute(model);
				
				model.addAttribute("nowPage", req.getParameter("noewPage"));
